...
function getGitAuthenticatedEnvironmentVariables(originalGitUrl, { token, username, password, hostType, matchHost }, environmentVariables) {
    if (!token && !(username && password)) {
        logger_1.logger.warn({ host: matchHost }, `Could not create environment variable for host as neither token or username and password was set`);
    ...
    if (token) {
        ...
        authenticationRules = getAuthenticationRules(originalGitUrl, hostType, `${encodedUsername}:${encodedPassword}`);
    }
...
function getGitEnvironmentVariables(additionalHostTypes = []) {
    ...
    const gitAllowedHostTypes = new Set([
        ...platforms_1.PLATFORM_HOST_TYPES,
        ...additionalHostTypes,
    ]);
