...
HARDCODED_EXCLUDE_ANNOTATIONS = {"config.runwhen.com/ignore": "true"}

HARDCODED_EXCLUDE_LABELS = {"runwhen-local": "ignore"}


HARDCODED_LOD_ANNOTATIONS = {"config.runwhen.com/lod": ["none", "basic", "detailed"]}
...
def get_lod_from_annotations(
    resource, lod_annotations: Dict[str, List[str]]
) -> Optional[LevelOfDetail]:
...
def has_included_annotations_or_labels(
    resource, include_annotations: Dict[str, str], include_labels: Dict[str, Any]
) -> bool:
    if not hasattr(resource, "metadata"):
        return False
    metadata = resource.metadata
    ...
    for key, values in include_labels.items():
        if key in labels:
            if (isinstance(values, list) and labels[key] in values) or (
                not isinstance(values, list) and labels[key] == values
            ):
...
def has_excluded_annotations_or_labels(
    resource, exclude_annotations: Dict[str, str], exclude_labels: Dict[str, Any]
) -> bool:
    if not hasattr(resource, "metadata"):
        return False
    metadata = resource.metadata
    ...
    for key, value in exclude_annotations.items():
        if annotations.get(key) == value:
            logger.info(
                f"Matched annotation {key} on {resource.metadata.name}, excluding from discovery."
            )
            return True
        ...
        if key in labels:
            if isinstance(values, list):
                if labels[key] in values:
                    logger.info(
                        f"Matched label {key} on {resource.metadata.name}, excluding from discovery."
                    )
                    return True
            else:
...
def index(component_context: Context):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        cloud_config_settings: Optional[dict[str, Any]] = component_context.get_setting(
            "CLOUD_CONFIG"
        )
        ...
        if cloud_config_settings:
            ...
            kubernetes_settings: Optional[dict[str, Any]] = cloud_config_settings.get(
                "kubernetes"
            )
            if kubernetes_settings:
                ...
                kubeconfig_text = base64.b64decode(encoded_kubeconfig_file).decode(
                    "utf-8"
                )
            ...
            if aks_settings:
                ...
                include_annotations = aks_settings.get(
                ...
                custom_namespace_names = aks_settings.get(
                    "namespaces", custom_namespace_names
                )
            ...
            if aks_settings:
                ...
                for cluster in clusters:
                    ...
                    default_namespace_lod = cluster.get(
                        "defaultNamespaceLOD",
                        (
                            default_lod
                            if default_lod == LevelOfDetail.NONE
                            else "detailed"
                        ),
                    )
                    if isinstance(default_namespace_lod, str):
                        default_namespace_lod = LevelOfDetail.construct_from_config(
                            default_namespace_lod
                        )
                    cluster_defaults[cluster_name] = default_namespace_lod

                    # Support namespaceLODs at the cluster level
                    cluster_namespace_lods[cluster_name] = cluster.get(
                        "namespaceLODs", {}
                    )
        ...
        if not kubeconfig:
            logger.info(
                "No Kubernetes configuration found, so skipping Kubernetes indexing"
            )
            return

        all_accessed_resource_type_specs = component_context.get_property(
            RESOURCE_TYPE_SPECS_PROPERTY
        )
        accessed_resource_type_specs = all_accessed_resource_type_specs.get(
            KUBERNETES_PLATFORM, dict()
        ).keys()
            ...
            for context in contexts:
                ...
                if extensions:
                    for ext in extensions:
                        if ext.get("name") == "workspace-builder":
                            ...
                            print(
                                f"Found workspace-builder extension for cluster '{cluster_name}': {extension_details}"
                            )
                ...
                if cluster:
                    ...
                    if extension_details:
                        ...
                        print(
                            f"Adding new cluster '{cluster_name}' with attributes including extension details: {cluster_attributes}"
                        )
                ...
                logger.info(
                    f"Scanning Kubernetes cluster {cluster_name} from context {context_name}"
                )

                with kubernetes_config.new_client_from_config(
                    config_file=kubeconfig_path, context=context_name
                ) as api_client:
                    core_api_client = client.CoreV1Api(api_client=api_client)
                    custom_objects_api_client = client.CustomObjectsApi(
                    ...
                    for group in ret.groups:
                        if group.name in group_version_infos:
                            logger.warning(
                                f"Duplicate group information in results from ApisApi.get_api_versions; "
                                f"group={group.name}"
                            )
                        versions = [
                            group_version.version for group_version in group.versions
                        ]
                        group_version_info = GroupVersionInfo(
                            group.preferred_version, versions
                        )
                    ...
                    try:
                        ...
                        for raw_resource in ret.items:
                            ...
                            if (
                                include_annotations or include_labels
                            ) and not has_included_annotations_or_labels(
                                raw_resource, include_annotations, include_labels
                            ):
                                continue  # Skip this resource if it doesn't meet inclusion criteria

                            if has_excluded_annotations_or_labels(
                                raw_resource, exclude_annotations, exclude_labels
                            ):
                                continue
                        ...
                        logger.info(
                            f"Error scanning for namespace instances; skipping and continuing; error: {e}"
                        )
                    ...
                    if len(namespace_names) == 0:
                        ...
                        project_version_info = group_version_infos.get(
                        ...
                        if project_version_info:
                            for version in project_version_info.versions:
                                try:
                                    ...
                                    # permissions error.
                                    logger.info(
                                        f"Error scanning for OpenShift project instances; "
                    ...
                    if len(namespace_names) == 0:
                        logger.warning(
                            "Unable to determine any namespace names, so can't index any Kubernetes resources"
                        )
                        return
                    ...
                    if custom_namespace_names:
                        namespace_names = namespace_names.intersection(
                    ...
                    for namespace_name in namespace_names:
                        namespace_qualified_name = get_qualified_name(
                        ...
                        default_namespace_lod = cluster_defaults.get(
                            cluster_name, default_lod
                        )

                        # Check for cluster-specific namespaceLODs first, then global ones
                        namespace_lod_config = cluster_namespace_lods.get(
                            cluster_name, {}
                        ).get(namespace_name, namespace_lods.get(namespace_name))
                        namespace_lod = (
                            LevelOfDetail.construct_from_config(namespace_lod_config)
                            if namespace_lod_config is not None
                            else default_namespace_lod
                        )

                        try:
                            logger.debug(
                                f"Attempting to read namespace: {namespace_name} in cluster: {cluster_name}"
                            )
                            raw_resource = core_api_client.read_namespace(
                                namespace_name
                            )
                            ...
                            if namespace_lod == default_namespace_lod:
                                namespace_lod = (
                                    get_lod_from_annotations(
                                        raw_resource, lod_annotations
                                    )
                                    or namespace_lod
                                )
                            ...
                            if namespace_lod == LevelOfDetail.NONE:
                                logger.debug(
                                    f"Skipping namespace {namespace_name} due to LevelOfDetail.NONE"
                                )
                                continue
                            if (
                                include_annotations or include_labels
                                ...
                                logger.debug(
                                    f"Skipping namespace {namespace_name} due to missing inclusion criteria"
                                )
                                continue  # Skip this resource if it doesn't meet inclusion criteria
                            if has_excluded_annotations_or_labels(
                                raw_resource, exclude_annotations, exclude_labels
                                ...
                                logger.debug(
                                    f"Skipping namespace {namespace_name} due to exclusion criteria"
                                )
                                continue

                            owner_name = extract_owner_name(raw_resource)
                            namespace_attributes = kubeapi_parsers.parse_namespace(
                                raw_resource
                            )
                    ...
                    for namespace in namespaces.values():
                        ...
                        logger.info(
                            f'Scanning for Kubernetes resources in namespace "{namespace_name}"'
                        )
                        ...
                        app_info = (
                            (
                                "Deployment",
                                apps_api_client.list_namespaced_deployment,
                                KubernetesResourceType.DEPLOYMENT,
                            ),
                            (
                                "DaemonSet",
                                apps_api_client.list_namespaced_daemon_set,
                                KubernetesResourceType.DAEMON_SET,
                            ),
                            (
                                "StatefulSet",
                                apps_api_client.list_namespaced_stateful_set,
                                KubernetesResourceType.STATEFUL_SET,
                            ),
                        )
                        ...
                        for (
                            ...
                            try:
                                ...
                                for raw_resource in ret.items:
                                    ...
                                    app_qualified_name = get_qualified_name(
                                        namespace_qualified_name, app_name
                                    )
                                    app_attributes = kubeapi_parsers.parse_app(
                                        raw_resource
                                    )
                                ...
                                logger.debug(
                                    f"Error scanning for deployment instances; skipping and continuing; error: {e}"
                                )
                        ...
                        networking_api_client = client.NetworkingV1Api(
                            api_client=api_client
                        )
                        try:
                            ret = networking_api_client.list_namespaced_ingress(
                                namespace_name
                            )
                            ...
                            for raw_resource in ret.items:
                                ...
                                ingress_qualified_name = get_qualified_name(
                                    namespace_qualified_name, ingress_name
                                )
                                ingress_attributes = kubeapi_parsers.parse_ingress(
                                    raw_resource
                                )
                                ...
                                for service_name in paths.values():
                                    service_qualified_name = get_qualified_name(
                                        namespace_name, service_name
                                    )
                                    ingresses_connected_services[
                                        service_qualified_name
                                    ] = ingress
                        except ApiException as e:
                            logger.info(
                                f"Error scanning for Ingress instances; skipping and continuing; error: {e}"
                            )
                        ...
                        try:
                            ...
                            ret = core_api_client.list_namespaced_service(
                                namespace_name
                            )
                            ...
                            for raw_resource in ret.items:
                                ...
                                service_qualified_name = get_qualified_name(
                                    namespace_qualified_name, service_name
                                )
                                service_attributes = kubeapi_parsers.parse_service(
                                    raw_resource
                                )
                                ...
                                if "ingress" not in service_attributes:
                                    ingress = ingresses_connected_services.get(
                                        service_qualified_name
                                    )
                                    service_attributes["ingress"] = ingress
                                service = registry.add_resource(
                                    KUBERNETES_PLATFORM,
                            ...
                            logger.info(
                                f"Error scanning for Service instances; skipping and continuing; error: {e}"
                            )
                        #
                        # Index the pvcs
                        #
                        try:
                            ret = (
                                core_api_client.list_namespaced_persistent_volume_claim(
                                    namespace_name, watch=False
                                )
                            )
                            logger.debug(
                                f"kube API scan for namespace {namespace_name}: {len(ret.items)} pvcs"
                            )
                            for raw_resource in ret.items:
                                if (
                                    include_annotations or include_labels
                            ...
                            logger.info(
                                f"Error scanning for PVC instances; skipping and continuing; error: {e}"
                            )
                        ...
                        for resource_type_spec in accessed_resource_type_specs:
                            ...
                            if version:
                                wildcard_spec = KubernetesResourceTypeSpec(
                                    KUBERNETES_PLATFORM,
                                    KubernetesResourceType.CUSTOM.value,
                                    group,
                                    None,
                            ...
                            try:
                                for version in versions:
                                    ...
                                    for raw_resource in ret["items"]:
                                        ...
                                        custom_qualified_name = get_qualified_name(
                                            namespace_qualified_name, custom_name
                                        )
                                        custom_attributes = (
                                            kubeapi_parsers.parse_custom_resource(
                                                raw_resource,
                                                group,
                                                version,
                                                plural_name,
                                        ...
                                        custom_resource = registry.add_resource(
                                            KUBERNETES_PLATFORM,
                                            KubernetesResourceType.CUSTOM.value,
                                            custom_name,
                                ...
                                logger.info(
                                    f"Error scanning for custom resource instances; skipping and continuing; "
