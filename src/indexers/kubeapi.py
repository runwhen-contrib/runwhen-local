...
def has_excluded_annotations_or_labels(resource, exclude_annotations: Dict[str, str], exclude_labels: Dict[str, str]) -> bool:
    ...
    for key, value in exclude_annotations.items():
        if annotations.get(key) == value:
            logger.info(f"Matched annotation {value} on {resource.metadata.name}, excluding from discovery.")
            return True
    
    for key, value in exclude_labels.items():
        if labels.get(key) == value:
            logger.info(f"Matched labeldf -h
             {value} on {resource.metadata.name}, excluding from discovery.")
            return True
